...
async function runCLI(
	...
	// JSON output for successful commands — skip if handler already emitted JSON
	if (jsonMode && !ctx.aborted && !ctx.jsonEmitted) {
		const jsonOutput: Record<string, unknown> = { status: "success", command: ctx.command, duration: ctx.duration };
		if (ctx.result !== undefined && ctx.result !== null) {
			jsonOutput.result = ctx.result;
		}
		console.log(JSON.stringify(jsonOutput));
	}

	// Compact mode output: if ctx.result is set, output compact format
	// Skip auto-emit if compactLine() was already called manually
	if (compactMode && !ctx.aborted && !ctx.compactEmitted && ctx.result !== undefined && ctx.result !== null) {
