...
const SYS_MSG = "You are Kapy, a human-first Digital Assistant. Be concise and direct.";
      ...
      for (let round = 0; round < MAX_ROUNDS; round++) {
        ...
        try {
          while (true) {
            ...
            for (const l of ls) {
              const d = l.startsWith("data: ") ? l.slice(6) : l; if (d === "[DONE]") break;
              try {
  ...
  return (
    <box width={dims().width} height={dims().height} backgroundColor="#1a1b26" flexDirection="column">
      <box flexDirection="row" flexGrow={1} minHeight={0}>
        <box flexGrow={1} minWidth={0}>
          <Show when={route.data().type === "home"}>
            <HomeScreen keyBindings={KB}
              ...
              inputRef={(r2: any) => { ref = r2; setTimeout(() => r2?.focus(), 10); }}
            />
          </Show>
        ...
        <Show when={sidebar()}>
          <Sidebar model={model} msgCount={msgs} models={models} />
        </Show>
      </box>
