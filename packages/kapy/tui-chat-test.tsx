...
import { createSignal, createEffect, createMemo, batch, For, Show, type ParentComponent, createContext, useContext } from "solid-js";
...
const SBCtx = createContext<{ open: () => boolean; toggle: () => void }>();
const SBP: ParentComponent = (props) => {
  const [o, setO] = createSignal(false);
  return <SBCtx.Provider value={{ open: () => o(), toggle: () => setO(v => !v) }}>{props.children}</SBCtx.Provider>;
};

const KB: KeyBinding[] = [{ name: "return", action: "submit" }, { name: "return", shift: true, action: "newline" }];
const DEFAULT_MODEL = "glm-5.1:cloud";
const SYS_MSG = "You are Kapy, a CLI assistant. Be concise.";
const SB_W = 36;
...
function KapyApp() {
  ...
  const handleKey = (evt: any) => {
    if (evt.ctrl && evt.name === "\\") { sb.toggle(); evt.preventDefault(); }
    if (evt.name === "escape") { if (isStreaming()) abort(); else if (route.data().type === "session") route.navigate({ type: "home" }); }
  };
  ...
  createEffect(() => { if (messages().length > 0) setTimeout(() => scrollRef?.scrollTo?.(scrollRef?.scrollHeight ?? 99999), 50); });

  const handleSlash = (t: string): boolean => {
    const cmd = t.trim().toLowerCase();
    if (cmd === "/sidebar" || cmd === "/sb") { sb.toggle(); return true; }
    return false;
  };
  ...
  return (
    <box width={dims().width} height={dims().height} backgroundColor="#1a1b26" flexDirection="column">
      <box flexDirection="row" flexGrow={1} minHeight={0}>
            ...
            <box flexDirection="column" alignItems="center" paddingLeft={2} paddingRight={2} flexGrow={1}>
              ...
              <box border={["left"]} borderColor="#00AAFF" width="100%" maxWidth={72}>
                <box paddingLeft={2} paddingRight={2} paddingTop={1} backgroundColor="#22223a">
                  <textarea focused placeholder='Ask anything... "Fix a TODO"' placeholderColor="#565f89" textColor="#c0caf5"
                    ...
                    ref={(r: any) => { homeRef = r; setTimeout(() => r?.focus(), 10); }}
                  />
          ...
          <Show when={route.data().type === "session"}>
            <box flexDirection="column" height="100%" paddingLeft={2} paddingRight={2}>
              <scrollbox ref={(r: any) => { scrollRef = r; }} flexGrow={1} minHeight={0}>
                ...
                <For each={messages()}>
                  {(m) => <Show when={m.role === "assistant"}>
                    <box paddingLeft={3} marginTop={1} flexShrink={0}>
                      <text fg="#c0caf5">{m.content}<Show when={m.streaming}><text fg="#00AAFF"> ●</text></Show></text>
                    </box>
                  </Show>}
                </For>
                <Show when={isStreaming()}><box paddingLeft={3} marginTop={1}><text fg="#00AAFF">● <text fg="#565f89">thinking...</text></text></box></Show>
                <box height={2} />
              </scrollbox>
              <Show when={error() !== null}><box paddingLeft={3} paddingBottom={1}><text fg="#f7768e">Error: {error()}</text></box></Show>
              <box flexShrink={0}>
                <box border={["left"]} borderColor="#00AAFF" width="100%">
                  <box paddingLeft={2} paddingRight={2} paddingTop={1} backgroundColor="#22223a">
                    <textarea focused placeholder="Message..." placeholderColor="#565f89" textColor="#c0caf5"
                      focusedTextColor="#c0caf5" focusedBackgroundColor="#22223a"
                      ...
                      ref={(r: any) => { sessRef = r; setTimeout(() => r?.focus(), 10); }}
                    />
                    <box flexDirection="row" paddingTop={1} gap={1}>
                      <text fg="#00AAFF">⟩</text>
                      <text fg="#c0caf5">kapy · {DEFAULT_MODEL}</text>
        ...
        <Show when={sb.open()}>
          <box backgroundColor="#1a1a2e" width={SB_W} height="100%" paddingTop={1} paddingBottom={1} paddingLeft={2} paddingRight={2} flexShrink={0}>
...
export async function launchChatTUI(): Promise<void> {
  ...
  await render(() => <SBP><RP><KapyApp /></RP></SBP>, renderer);
}
